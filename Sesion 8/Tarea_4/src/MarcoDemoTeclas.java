...
public class MarcoDemoTeclas extends JFrame implements KeyListener {
    ...
    public void keyPressed(KeyEvent evento)
    {
        ...
        if(KeyEvent.getKeyText(evento.getKeyCode()).equals("R")){
            areaTexto.setBackground(Color.red);
        }
        if(KeyEvent.getKeyText(evento.getKeyCode()).equals("V")){
            areaTexto.setBackground(Color.green);
        }

        if(evento.isActionKey()) {
            if (KeyEvent.getKeyText(evento.getKeyCode()).equals("F1")) {
                areaTexto.setBackground(Color.white);
            }
            if (KeyEvent.getKeyText(evento.getKeyCode()).equals("F2")) {
                areaTexto.setBackground(Color.yellow);
            }
        }

        if(evento.getKeyChar() == '/'){
            JOptionPane.showMessageDialog(ventana1, "No hay color");
        }
